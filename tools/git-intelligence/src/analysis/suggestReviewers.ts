...
const SENSITIVE_ROLE_MAP: Array<{ path: string; role: string }> = [
...
function parseBlame(blame: string, scores: Map<string, AuthorScore>): void {
  ...
  for (const line of lines) {
    if (line.startsWith('author-mail ')) {
      ...
      if (!currentEmail || currentEmail === 'not.committed.yet') continue;
...
export async function suggestReviewers(git: SimpleGit, diff: ParsedDiff): Promise<string[]> {
  const scores = new Map<string, AuthorScore>();
  ...
  if (topAuthors.length >= 2) return topAuthors;

  // Sensitive-path role fallback when not enough blame authors found
  const roleFallbacks: string[] = [];
