import type { ParsedDiff, RiskReport, AnalysisOptions } from '../types.js';
...
function computeHeuristicScore(diff: ParsedDiff): RiskReport {
  let score = 1;
  ...
  const sensitiveHit = new Set<string>();
...
function parseLLMRisk(raw: string): RiskAdjustment | null {
  try {
    return JSON.parse(raw) as RiskAdjustment;
  } catch {
    return null;
  }
}

export async function detectRiskyPR(diff: ParsedDiff, options: AnalysisOptions): Promise<RiskReport> {
