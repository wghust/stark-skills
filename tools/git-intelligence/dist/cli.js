...
function printCommitSummary(summary, since, format) {
    if (format === 'json') {
...
function printRiskReport(report, branch, format) {
    if (format === 'json') {
...
function printImpactReport(report, branch, format) {
    if (format === 'json') {
        print(JSON.stringify({ branch, ...report }, null, 2));
        return;
    }
...
function printPRAnalysis(analysis, format) {
    if (format === 'json') {
...
program
    ...
    const options = { useLLM: parent.llm, output: parent.output };
    try {
