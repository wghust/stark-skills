...
function truncate(text, label = 'items') {
...
function formatCommits(commits) {
    return truncate(commits.map((c, i) => `${i + 1}. ${c.message} (by ${c.author})`).join('\n'), 'commits');
}
...
export function buildCommitSummaryPrompt(commits) {
    return {
...
export function buildReleaseNotesPrompt(commits, from, to) {
    return {
...
export function buildPRAnalysisPrompt(diff, branch) {
    return {
...
export function buildRiskPrompt(diff, heuristicScore) {
    return {
...
export function buildImpactPrompt(diff) {
    return {
