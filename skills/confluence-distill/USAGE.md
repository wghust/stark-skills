...
- 建议显式给出 **space 范围** 和 **页数预算**（默认 20，单批建议不超过 50）。
- 如果你说“越全越好”，代理会先要求你确认分批方案（按空间或子主题），不会直接全量抓取。
...
- 推荐从 1-2 请求/秒起步；遇到 429 时按 2s/4s/8s/16s（上限 30s）退避重试。
...
- Confluence 页面正文可能包含凭据/PII；蒸馏时应做最小必要摘录，必要处用 `[REDACTED]` 并保留 source-map 可追溯链接。
