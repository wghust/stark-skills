...
## 方形画布与非方形源图（contain / cover）

Favicon 的每一档都是 **正方形**像素。源图若不是 1:1，必须把图放进方框里：

- **默认 `contain`**：整张图缩小后 **完整可见**，两侧或上下 **留白**；有 alpha 时常为 **透明** 边，无 alpha（如 JPEG）时 CLI 使用 **白边**（见 `USAGE.md`）。这样图标在观感上仍是用户提供的同一枚图形，而非被裁成另一构图。
- **`cover`（仅用户明确要求）**：铺满方形，**裁掉**超出部分，可能改变可见内容。
